        ...
        <div class="flex items-center justify-between mb-4">
          ...
          <!-- confirmi conditioon v if essq ticket_direction.direction == cab ou commercial ou type orientation == ticket ou default -->
            <!-- <q-btn
              ...
              color="green"
              size="sm"
              v-if="(  isPresidentRecours && canAddDirection)"
              round
          ...
          <!-- confirmi conditioon v if essq cab ou commercial ou type orientation == ticket ou default -->
            <!-- <q-btn
              ...
              size="sm"
              round
              v-if="( isPresidentRecours && canRemoveDirection)"
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div class="flex justify-end gap-3">
            ...
            <q-btn
              ...
              color="green-6"
              no-caps
              v-if="isPresidentRecours && canShowReplyButton"
        ...
        <q-card-actions class="p-6 bg-gray-50">
          ...
          <q-btn
            ...
            v-if="(selectedMessage.destinataires[0].direction_destinataire!='client'
              && selectedMessage.destinataires[0].direction_destinataire!='directions'
              && ticket.status!='clôturé' && ticket.status!='Recours clôturé')
...
const showAssociatedDirections = computed(() => !!currentTicketId.value && directionOptions.value.length > 0 )
const canRemoveDirection = computed(() => ticket.value?.status!='clôturé' && ticket.value?.status!='Recours clôturé')
const isLoadingDirections = computed(() => loadingDirections.value)

const canShowConclusionButton = computed(() =>
  ['clôturé','Recours clôturé'].includes(ticket.value?.status)
  // Le président de la commission peut consulter la conclusion pilote
  // pendant que le recours est encore ouvert (avant clôture).
  || (isPresidentRecours.value && ticket.value?.status === 'Recours')
)
const canShowNewMessageButton = computed(() => ticket.value?.status!='clôturé' && ticket.value?.status!='Recours clôturé')
const canShowReplyButton = computed(() => (
   canClientReply.value &&
...
// Chargement combiné: messages + directions en un seul appel
const loadMessagesDirections = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      // Messages et métadonnées du ticket
      ...
      // Directions concernées et non concernées (mêmes mappings que loadDirections)
      ...
      // Conserver comportement existant en cas d'absence de données
      messages.value = []
      directionOptions.value = []
      directionsNonConcerneOptions.value = []
      $q.notify({
        type: 'warning',
        message: response.data.message || 'Aucun message ou direction trouvé'
      })
    }
  } catch (error) {
    console.error('Erreur lors du chargement combiné messages+directions:', error)
    messages.value = []
    // Fallback directions comme dans loadDirections
    directionOptions.value = [
    ...
    $q.notify({
      type: 'negative',
      message: 'Erreur lors du chargement des messages et directions'
    })
  } finally {
    loading.value = false
...
onMounted(() => {
  if (currentTicketId.value) {
    ...
    // Charger au moins les directions par défaut si aucun ticket
    loadDirections()
  }
})
